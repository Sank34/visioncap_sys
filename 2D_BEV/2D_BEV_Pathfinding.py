...
with dai.Device(pipeline) as device:
    ...
    while True:
        ...
        if binary_map[start[0], start[1]] != 1 or binary_map[goal[0], goal[1]] != 1:
            print("[WARN] Start or goal is blocked.")
            path = []
        else:
            path = bfs(binary_map, start, goal)
        ...
        bev_scaled = cv2.resize(bev_color, (640, 640), interpolation = cv2.INTER_NEAREST)
        cv2.imshow("Bird's Eye View Occupancy Map", bev_scaled)
