...
LOW_RISK_LABELS = {'bottle', 'cat', 'pottedplant', 'bird'}  # all others = high risk
...
with dai.Device(pipeline) as device:
    ...
    while True:
        ...
        for name in ROI_NAMES:
            high_risk_close = [obj for obj in roi_stats[name] if obj[1] == "high" and obj[2] < 3.0]
            scores[name] = len(high_risk_close)

        print("\n--- ROI SUMMARY ---")
        for name in ROI_NAMES:
            objects = roi_stats[name]
            high = [obj for obj in objects if obj[1] == "high"]
            low = [obj for obj in objects if obj[1] == "low"]
            print(f"[{name.upper()}] High-risk: {len(high)} | Low-risk: {len(low)}")
            for obj in high:
                print(f"   - {obj[0]} at {obj[2]:.2f} m")
        ...
        print(f"[DECISION] Final: {direction}")

        for i in range(1, NUM_ROIS):
            x = i * roi_width
            cv2.line(frame, (x, 0), (x, frame.shape[0]), (255, 255, 0), 2)

        cv2.putText(frame, f"Decision: {direction}", (10, 25), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 255), 2)
