...
# Global goal cell
goal_cell = [0, grid_cols // 2]
occupancy_grid = np.zeros((grid_rows, grid_cols), dtype=np.uint8)

def on_mouse(event, x, y, flags, param):
    if event == cv2.EVENT_LBUTTONDOWN:
        col = x * grid_cols // image_width
        row = y * grid_rows // image_height
        print(f"!!!!!!! Clicked grid: ({row}, {col}), Value: {occupancy_grid[row, col]}")
        goal_cell[0] = row
        goal_cell[1] = col
        print(f"!!!!!!! New goal selected at: row={row}, col={col}")
...
cv2.resizeWindow("Occupancy Grid", image_width, image_height)
cv2.setMouseCallback("Occupancy Grid", on_mouse)
...
with dai.Device(pipeline) as device:
    ...
    while True:
        ...
        if not path:
            print("No path found to selected goal.")
        else:
        ...
        # Draw the goal cell as red dot
        gx = goal_cell[1] * cell_width + cell_width // 2
        gy = goal_cell[0] * cell_height + cell_height // 2
        gx = int(np.clip(gx, 0, image_width - 1))
        gy = int(np.clip(gy, 0, image_height - 1))
        occ_vis = cv2.resize(occ_vis, (image_width, image_height), interpolation=cv2.INTER_NEAREST)
        cv2.circle(occ_vis, (gx, gy), 4, (0, 0, 255), -1)

        cv2.imshow("Occupancy Grid", occ_vis)
