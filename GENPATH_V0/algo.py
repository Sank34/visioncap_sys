...
with dai.Device(pipeline) as device:
    ...
    while True:
        ...
        occ_vis = (occupancy_grid * 255).astype(np.uint8)
        occ_vis = cv2.resize(occ_vis, (image_width, image_height), interpolation=cv2.INTER_NEAREST)
        occ_vis = cv2.applyColorMap(occ_vis, cv2.COLORMAP_BONE)

        combined = np.hstack((depthColor, occ_vis))
        cv2.imshow("Depth | Occupancy Grid", combined)
        ...
        bottom_strip = depthFrame[-N :, :]
        strip_vis = depthFrame[-N :, :]
        strip_vis = cv2.normalize(strip_vis, None, 0, 255, cv2.NORM_MINMAX).astype(np.uint8)
        strip_vis = cv2.applyColorMap(strip_vis, cv2.COLORMAP_JET)
        strip_vis = cv2.resize(strip_vis, (image_width, 50))  # optional: stretch vertically for clarity
        cv2.imshow("Ground Estimation Strip", strip_vis)

        pitch_deg = math.degrees(pitch_rad)

        # print(f"Pitch angle: {pitch_deg:.2f}")
        # print(f"Estimated ground depth from strip: {ground_depth:.1f} mm")
